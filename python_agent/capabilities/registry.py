...
6. `gradient` 默认 false（加速渲染）；资讯/应用均建议 false。
7. **intro_card / outro_card**：抖音与小红书默认 **intro_card=true**（片头展示 hook）；抖音可 outro=true。
""".strip()
...
def merge_render_effects(
    ...
    *,
    ...
    if fk in ("news", "apps"):
        gradient = False
    # 资讯/应用：句级字幕走 ASS（caption_remotion=false），片头片尾仍可用 Remotion TitleCard/CTA
    caption_remotion = remotion_on
    if fk in ("news", "apps"):
        caption_remotion = False
    ...
    effects: dict[str, Any] = {
        ...
        "caption_remotion": caption_remotion,
        "preset": preset_name,
        ...
        "intro_card": bool(pe.get("intro_card", preset_spec.get("intro_card", True))),
        "outro_card": bool(pe.get("outro_card", preset_spec.get("outro_card", platform_id == "douyin"))),
