...
def list_ai_news_variants() -> list[dict[str, Any]]:
    return list(load_ai_news_catalog().get("variants") or [])
...
def pick_ai_news_template(brief: dict[str, Any]) -> dict[str, Any]:
    """按 article_id / content_key 稳定轮换款式（非每镜随机）。"""
...
def apply_pinned_ai_news_plan(
    ...
    bgs = list(title_cfg.get("background_colors") or ["#0f172a", "#1e293b"])
    ...
    for s in slides:
        ...
        if st == "title_card":
            ...
            ci += 1
            si = (ci - 1) % max(len(bgs), 1)
            slide["background_color"] = bgs[si % len(bgs)]
