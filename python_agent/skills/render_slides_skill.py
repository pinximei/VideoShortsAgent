...
class RenderSlidesSkill:
    ...
    def execute(self, slides: list, tts_clips: list,
        ...
        for i, slide in enumerate(slides):
            ...
            if tts_audio_path and os.path.exists(tts_audio_path):
                audio_ok = self._attach_audio(video_path, tts_audio_path, segment_path)
                if not audio_ok:
                    print(f"  [Slide {i+1}] ⚠️ 音频附加失败，添加静音音轨")

            if not audio_ok:
                self._add_silent_audio(video_path, duration, segment_path)
    ...
    def _render_with_remotion(self, slide, style, frames, sentences,
        ...
        if slide.get("image_path"):
            props["imagePath"] = slide["image_path"].replace("\\", "/")
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, text=True,
                                    timeout=300, cwd=REMOTION_DIR, shell=True)
    ...
    def _concat_slides(self, segment_paths, slides, output_path, output_dir):
        ...
        if len(segment_paths) == 2:
            ...
            filter_parts.append(
                f"[0:v][1:v]xfade=transition={transitions[0]}:duration={transition_duration}:offset={offset}[vout]")
            ...
            for i in range(len(segment_paths) - 1):
                ...
                filter_parts.append(
                    f"{inv}[{i+1}:v]xfade=transition={transitions[i]}:duration={transition_duration}:offset={max(0,off)}{outv}")
    ...
    def _mix_bgm(self, video_path, bgm_path, output_path, bgm_volume=0.15):
        """将 BGM 混入视频"""
        ...
        if has_audio:
            fc = (f"[1:a]volume={bgm_volume},afade=t=in:st=0:d=2,"
                  f"afade=t=out:st={fade_out_start}:d=3[bgm];"
