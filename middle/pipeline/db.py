...
class JobStore:
    ...
    def upsert_discovered(self, *, content_key: str, article_id: int, snapshot: dict[str, Any]) -> bool:
        now = _utc_now()
        ...
        with self._conn() as conn:
            cur = conn.execute(
                """
                INSERT INTO jobs (content_key, article_id, status, step, soul_snapshot, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(content_key) DO NOTHING
                """,
                (content_key, article_id, STATUS_DISCOVERED, "discover", snap, now, now),
            )
            created = cur.rowcount > 0
        if created:
            self.log_event(content_key, status=STATUS_DISCOVERED, step="discover", message="Soul new article")
        return created
    ...
    def pending_publish(self, channel: str) -> list[dict[str, Any]]:
        with self._conn() as conn:
            rows = conn.execute(
                """
                ...
                """,
                (channel, STATUS_READY, STATUS_PROCESSING),
            ).fetchall()
            return [dict(r) for r in rows]
