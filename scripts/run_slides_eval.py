...
def _eval_brief(brief: dict, *, platform: str) -> dict:
    ...
    slides = auto_fix_slides(slides, platform=platform)

    # 模拟 TTS 句轴
    mock_clips = []
    t = 0.0
    for s in slides:
        phrases = split_spoken_clauses(str(s.get("tts_text") or ""), max_chars=18)
        sents = []
        for p in phrases:
            dur = max(0.8, len(p) * 0.14)
            sents.append({"text": p, "start": t, "end": t + dur})
            t += dur + 0.2
...
def main() -> int:
    ...
    for brief in _load_fixtures():
        for plat in platforms:
            print(f"[eval] {brief.get('id')} / {plat}")
            results.append(_eval_brief(brief, platform=plat))
    ...
    cap_issues = sum(len(c.get("issues") or []) for r in results for c in r.get("captions") or [])
    print(f"\nWrote {out_path}")
